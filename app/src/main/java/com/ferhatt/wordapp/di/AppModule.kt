package com.ferhatt.wordapp.di

import android.content.Context
import androidx.room.Room
import com.bumptech.glide.Glide
import com.bumptech.glide.request.RequestOptions
import com.ferhatt.wordapp.R
import com.ferhatt.wordapp.api.RetrofitAPI
import com.ferhatt.wordapp.repo.WordRepoInterface
import com.ferhatt.wordapp.repo.WordRepository
import com.ferhatt.wordapp.roomdb.WordDao
import com.ferhatt.wordapp.roomdb.WordDatabase
import com.ferhatt.wordapp.util.Util.BASE_URL
import dagger.Module
import dagger.Provides
import dagger.hilt.InstallIn
import dagger.hilt.android.qualifiers.ApplicationContext
import dagger.hilt.components.SingletonComponent
import retrofit2.Retrofit
import retrofit2.converter.gson.GsonConverterFactory
import javax.inject.Singleton

@Module
@InstallIn(SingletonComponent::class)
object AppModule {

    @Singleton
    @Provides
    fun injectRoomDatabase(
        @ApplicationContext context : Context) = Room.databaseBuilder(
        context,WordDatabase::class.java,"WordBookDB").build()

    @Singleton
    @Provides
    fun injectDao(database: WordDatabase) = database.wordDao()

    @Singleton
    @Provides
    fun injectNormalRepo(dao: WordDao,api: RetrofitAPI) = WordRepository(dao,api) as WordRepoInterface



    @Singleton
    @Provides
    fun injectRetrofitAPI() : RetrofitAPI {
        return Retrofit.Builder().addConverterFactory(GsonConverterFactory.create())
            .baseUrl(BASE_URL)
            .build()
            .create(RetrofitAPI::class.java)
    }


    @Singleton
    @Provides
    fun injectGlide(@ApplicationContext context: Context) = Glide.with(context)
        .setDefaultRequestOptions(
            RequestOptions().placeholder(R.drawable.ic_launcher_foreground)
                .error(R.drawable.add)
        )



}